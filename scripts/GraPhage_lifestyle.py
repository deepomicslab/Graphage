"""GraPhage_lifestyle 
Usage:
    GraPhage_lifestyle.py <fasta_file> [--out=<fn>] [--thread=<tn>] [--batch=<bn>]
    GraPhage_lifestyle.py (-h | --help)
    GraPhage_lifestyle.py --version
Options:
    -o --out=<fn>   The output file name [default: GraPhage_lifestyle_output.txt].
    ...
    for data in loader:
        with torch.no_grad():
            ...
            for each in pred:
                ...
                count += 1
    f.close()
    print("Finish. The result can be find at %s. Thank you for using GraPhage."%output_file)

if __name__=="__main__":
    arguments = docopt(__doc__, version="GraPhage_lifestyle 0.1.1")
    main(arguments)
